-- ====================================================================
-- ConvertCast Event Notification System Migration (FINAL VERSION)
-- Version: 1.0.0
-- Date: 2025-01-05
-- Description: Complete event scheduling, registration, and notification system
-- Safe for TEXT-based status columns (no enum modifications)
-- ====================================================================
...
-- Note: 'cancelled' status is already supported as TEXT value - no enum modification needed
...
-- Trigger for event_notifications (check if exists first)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'update_event_notifications_updated_at'
  ) THEN
...
-- Trigger for event_analytics (check if exists first)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'update_event_analytics_updated_at'
  ) THEN
...
-- Create trigger on events table (check if exists first)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'create_event_analytics_trigger'
  ) THEN
...
-- Enable RLS on event_notifications (safe if already enabled)
ALTER TABLE event_notifications ENABLE ROW LEVEL SECURITY;

-- Create policies (drop and recreate to ensure correct version)
DO $$
BEGIN
...
-- Create policies for event_analytics
DO $$
BEGIN
...
-- Drop and recreate view (safe to replace)
...
-- Function to track notification clicks
